...
This code is a bit messy, and does things it doesn't need to - it took me a while to figure things
out.
...
fn find_ms(n: u64, max_depth: Option<usize>) -> HashMap<u64, Vec<(u64, u64, u64)>> {
    ...
    loop {
        ...
        for &(n2, _, _) in last.path.iter() {
            ...
            if last.path.len() >= maxn {
                continue;
            }
            match max_depth {
                Some(d) if d < last.path.len() => {
                    continue;
                }
                _ => {}
            }
...
pub fn main() {
    let bigm = 200;
    let ms = find_ms(bigm, Some(11));
    // 11 found by trial and error; 10 doesn't get everything, 11 works, so take 11...
